...
uint8_t PMU_Get_24M_Clock_Source_div(PMU_TypeDef *pmu)
{
    ...
    uint8_t div = 0;
    if (!(pmu->CTL & PMU_PMU_CTL_OSC_SEL_Msk))
    {
        div = 1;
    }
    else
...
void PMU_Set_24M_Clock_Source_div(PMU_TypeDef *pmu, uint8_t div)
{
    ...
    {
        case 1:
            pmu->CTL &= ~PMU_PMU_CTL_OSC_SEL_Msk;
            break;
        case 2:
            pmu->CTL |= PMU_PMU_CTL_OSC_SEL_Msk;
...
void PMU_Set_Rtc_Clock_Source(PMU_TypeDef *pmu, uint8_t select_rtc_clock)
{
    ...
    uint8_t data = pmu->CLK;
    data |= PMU_PMU_CLK_TCLK_EN_Msk;
    data &= ~PMU_PMU_CLK_RTCCLK_SEL_Msk;
    data |= select_rtc_clock;
    pmu->CLK = data;
}
...
void PMU_Set_Wdt_Clock_Source(PMU_TypeDef *pmu, uint8_t select_wdt_clock)
{
    ...
    uint8_t data = pmu->CLK;
    data |= PMU_PMU_CLK_TCLK_EN_Msk;
    data &= ~PMU_PMU_CLK_WDTCLK_SEL_Msk;
    data |= select_wdt_clock;
    pmu->CLK = data;
}
...
void PMU_Set_NPU_Clock_Div_Num(PMU_TypeDef *pmu,uint8_t div_num)
{
    ...
    uint8_t data = pmu->CLK_CTL;
    data &= ~PMU_CLK_CTL_NPU_DIV_NUM_Msk;
    data |= (div_num << PMU_CLK_CTL_NPU_DIV_NUM_Pos);
    pmu->CLK_CTL = data;
}
