...
__attribute__((weak)) void TIMER0_IRQHandler(void)
{
    ...
    hal_i2s_instance->timer_timeout_ms_flag = 1;
}
...
int hal_i2s_init(Hal_I2s_InitTypeDef *i2s_instance,I2S_TypeDef *address,Hal_I2s_Type_Typedef type,Hal_I2s_Work_Mode_Typedef mode,Hal_I2s_Width_Word_Typedef width,int lr_channel_need_sizes_by_width)
{
    ...
    hal_i2s_instance->timer_timeout_ms_flag = 0;
...
int hal_i2s_ctl(Hal_I2s_InitTypeDef *i2s_instance,int command, ...)
{
    ...
    switch(command)
    {
        ...
        case HAL_I2S_CHANNEL_SET_TIMER_TIMEOUTS_MS:
        {
            value = va_arg(ParamList, int);
            uint32_t tick = 0;
            ...
            tick = TIMER_Transfer_millisecond_to_tick(TIMER0,value);
        ...
        case HAL_I2S_CHANNEL_START_TIMER:
        {
            value = va_arg(ParamList, int);
            TIMER_Set_Enable_Cmd(TIMER0, value);
            break;
        }
    
        case HAL_I2S_CHANNEL_GET_TIMER_TIMEOUS_MS_FLAG:
        {
            return i2s_instance->timer_timeout_ms_flag;
        }
        
        case HAL_I2S_CHANNEL_SET_TIMER_TIMEOUS_MS_FLAG:
        {
            value = va_arg(ParamList, int);
            i2s_instance->timer_timeout_ms_flag = value;
        }
...
int hal_i2s_write(Hal_I2s_InitTypeDef *i2s_instance,void *left_data,void *right_data,int size_by_data)
{
    ...
    if(i2s_instance->width_word == HAL_I2S_16BITS_WIDTH_WORD)
    {
        ...
        if(i2s_instance->send_buffer.read_index + size_by_data * 2 * i2s_instance->width_word <= i2s_instance->send_buffer.lr_channel_need_sizes_by_width_counts * i2s_instance->lr_channel_need_sizes_by_width * i2s_instance->width_word)
        {
        ...
        {
            int over = i2s_instance->send_buffer.read_index + size_by_data * 2 * i2s_instance->width_word - i2s_instance->send_buffer.lr_channel_need_sizes_by_width_counts * i2s_instance->lr_channel_need_sizes_by_width * i2s_instance->width_word;
            int under = i2s_instance->send_buffer.lr_channel_need_sizes_by_width_counts * i2s_instance->lr_channel_need_sizes_by_width * i2s_instance->width_word - i2s_instance->send_buffer.read_index;
            int over_len = over / i2s_instance->width_word / 2;
            int under_len = under / i2s_instance->width_word / 2;
            ...
            i2s_buffer16 = (uint16_t *)(&i2s_instance->send_buffer.buffer[0]);
            for(int i = 0 ;i < over_len; i++)
            {
                if(left_temp16)
                 i2s_buffer16[i * 2] =  left_temp16[i + under_len];
                if(right_temp16)
                 i2s_buffer16[i * 2 + 1] = right_temp16[i + under_len];
            }
        }
    }
    else
    {
        ...
        else
        {
            ...
            i2s_buffer32 = (uint32_t *)(&i2s_instance->send_buffer.buffer[0]);
            for(int i = 0 ;i < over_len; i++)
            {
                if(left_temp32)
                 i2s_buffer32[i * 2] =  left_temp32[i + under_len];
                if(right_temp32)
                 i2s_buffer32[i * 2 + 1] = right_temp32[i + under_len];
            }
        }
    }

    i2s_instance->send_buffer.read_index += size_by_data * 2 * i2s_instance->width_word;
