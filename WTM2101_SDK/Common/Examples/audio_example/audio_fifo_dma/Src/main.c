/**
  ******************************************************************************
  * @file    main.c 
  * @brief   This example describes sending read data in real time 
             through I2S1 via FIFO in AUDIO channel 0.
  * @date    2023-02-07
...
volatile Bool audio_buff_prepare = FALSE;
...
void main(void)
{
    ...
    while(1)
    {
        if (audio_buff_prepare == TRUE)
        {
            audio_buff_prepare = FALSE;
            ...
            while (audio_dma_completed != TRUE);
            ...
            for (uint8_t i = 0; i < DATA_BUFF_LEN; i++)
            {
                ...
                audio_buff[i << 1] = tmp >> 16;
                audio_buff[(i << 1) + 1] = tmp;
            }
#endif
