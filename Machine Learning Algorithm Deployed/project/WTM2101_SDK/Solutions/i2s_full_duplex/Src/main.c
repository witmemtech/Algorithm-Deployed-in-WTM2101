...
static int init_example(void)
{
    ...
    ret = hal_i2s_init(hal_i2s_instance1,I2S0,HAL_I2S_RECEIVE_AND_SEND,HAL_I2S_SLAVE,HAL_I2S_32BITS_WIDTH_WORD,320);
    if(ret <= 0)
    {
        printf("hal_i2s_init is error:%d\r\n",ret);
        return -5;
...
static int start_example(void)
{
    ...
    extern  void hal_i2s_gpio_init(Hal_I2s_InitTypeDef *i2s_instance);
    hal_i2s_gpio_init(hal_i2s_instance1);
...
static int data_handle_example(void)
{
    ...
    if(data_flag2)
    {
        ...
        hal_i2s_write(hal_i2s_instance,temp2,temp2,hal_i2s_instance->lr_channel_need_sizes_by_width / 2);
    }
}
