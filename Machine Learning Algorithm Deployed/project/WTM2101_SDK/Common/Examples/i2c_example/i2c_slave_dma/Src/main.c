...
uint8_t writeBuff[BUFFER_SIZE];
uint8_t readBuff[sizeof(writeBuff) + 1];
volatile int Dma_Transfer_Interrupt_Flag = 0;

void DMA_IRQHandler(void)
{
    int Transfer_Interrupt_Flag = DMA_Get_Transfer_Interrupt_Status(DMA);

    if (Transfer_Interrupt_Flag & DMA_CHANNEL0)
    {
        DMA_Clear_Transfer_Interrupt_Cmd(DMA, DMA_CHANNEL0);
        Dma_Transfer_Interrupt_Flag = 1;
    }

    if (Transfer_Interrupt_Flag & DMA_CHANNEL1)
    {
...
void main(void)
{
    ...
    /* Application entry */
    printf("\r\nstart test,wait i2c master write and read data\r\n");
    ...
    for (int i = 0; i < sizeof(writeBuff) + 1; i++)
    {
        printf(" %X",readBuff[i]);
    }
...
static void mem_to_i2c_init(uint8_t chlx)
{
    ...
    DMA_Set_Transfer_Interrupt_Cmd(DMA, chlx, ENABLE);
}
...
static void write_dma(uint8_t *pbuffer, uint8_t addr, uint8_t numbyte)
{
    ...
    while (I2C_IT_GetRAWStatBit(I2C, I2C_IT_RD_REQ) == RESET) {}
...
static int i2c_test(void)
{
    ...
    read_dma(readBuff, 0x00, sizeof(readBuff) + 1);

    while (!Dma_Transfer_Interrupt_Flag) {}
    ...
    return memcmp(readBuff + 1, writeBuff, sizeof(writeBuff));
}
