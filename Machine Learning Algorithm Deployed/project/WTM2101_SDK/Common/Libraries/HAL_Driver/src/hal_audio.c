...
#pragma GCC optimize("no-reorder-blocks,no-schedule-insns,no-schedule-insns2,no-store-merging")
#pragma GCC optimize("no-inline,no-inline-functions,no-inline-small-functions")

//#define HAL_AUDIO_INSTANCE_NUMBER 2  // yxp
...
static int HAL_Audio_AnalogInit(Audio_InitTypeDef *audio_instance)
{
    ...
    */
    ...
    // bugfixed-2024.03.28: config analog adc twice will FATAL...
    // so we reset it at the first time.
    // config bitset second time.
    // begin -----------------------------------------------
    if (audio_instance->audio_rst) {
        val = 0;
        ...
        val = audio_instance->instance->ANA0CFG;
    }


    //
    //  ADC0 ADC1 module global config
    //
    ...
    if (audio_instance->InputType == HAL_AUDIO_MIC_INPUT_AMIC) {
        ...
        // bugfixed-2024.03.15: using vmid, not bg
        // BIAS_REF_SEL: 0= using vmid, 1= using bg
        // begin
        val &= ~AUDIO_ANA0CFG_BIAS_REF_SEL_O_Msk;
        // end
        val |=  AUDIO_ANA0CFG_BIAS_OUT_EN1_O_Msk;

        val &= ~AUDIO_ANA0CFG_BST_SEL_CH0_O_Msk;
    ...
    //
    if (audio_instance->InputType == HAL_AUDIO_MIC_INPUT_LINEIN) {
        val &= ~AUDIO_ANA0CFG_BST_SEL_CH1_O_Msk;
    ...
    //  4= 0.6V
    //
    val &= ~AUDIO_ANA0CFG_VCOM_SEL_O_Msk;
    ...
    * |pu_bg_o                        = 1
    */
    // bugfixed-2024.03.28: see above comment
    // begin -----------------------------------------------
    if (audio_instance->audio_rst){
    val = 0;
    } else {
        val = audio_instance->instance->ANA1CFG;
    }
    ...
    */
    ...
    val &= ~AUDIO_ANA1CFG_IREF_TRIM_O_Msk;
...
int hal_audio_ana_init(Audio_InitTypeDef *audio_instance){
...
int hal_audio_init(Audio_InitTypeDef *audio_instance,Audio_Mic_Input_TypeDef type)
{
    ...
    audio_instance->channel.ChannelGain = 10;
...

    {
        ...
        audio_instance->Analog.LinBoostGain = HAL_AUDIO_ANALOG_LIN_BOOST_M12DB;
...

    {
        ...
        audio_instance->dma_channel = DMA_CHANNEL4;
...
int hal_audio_open(Audio_InitTypeDef *audio_instance)
{
    ...
    if(audio_instance->InputType != HAL_AUDIO_MIC_INPUT_DMIC)
    {
        ...
        else  
        {
            ...
            AUD->ANA1CFG = 0x08000301;
            AUD->ANA2CFG = 0x00010007;
            AUD->ANA19CFG = 0x00137F84;
        }
