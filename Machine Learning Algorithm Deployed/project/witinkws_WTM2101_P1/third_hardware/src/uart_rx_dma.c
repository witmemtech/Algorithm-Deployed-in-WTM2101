...
#if defined(UART_ENABLE) && (defined(UART1RX_DMA_CHANNEL)||defined(UART0RX_DMA_CHANNEL))
...
void UART_RecvFIFO_Start(UART_TypeDef* UARTx)
{
    ...
    if (UARTx == UART0) {
        chl = UART0RX_DMA_CHANNEL;
    } 
#endif
#ifdef UART1RX_DMA_CHANNEL
    if (UARTx == UART1) {
        chl = UART1RX_DMA_CHANNEL;
    }
#endif
    ...
    //
    for (i = 0;i < nr_cfg;i++) {
        _rx_dma_llp[i].src          = (uint32_t)&(UARTx->DR_DLL);
    ...
    UART_IT_Cfg(UARTx, UART_IT_ERBFI, ENABLE);
}
...
#endif
    DMA_Set_Channel_Enable_Cmd(DMA, chl, DISABLE);
    UART_IT_Cfg(UARTx, UART_IT_ERBFI, DISABLE);
}
