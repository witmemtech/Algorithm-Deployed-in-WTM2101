...
#define APPLINK_FRAME_SIZE      1024// 1024*int16_t, maximal frame size
...
int uartxfer_callback_debug(char* buffer, int buflen);
void uartxfer_callback_newfile(void);
void uartxfer_callback_reset(void);
void uartxfer_callback_process(int16_t* in, int16_t* out, int nr_bytes);


int jtagxfer_sendrecv_msg(const datalink_message_t* txmsg, datalink_message_t* rxmsg);
