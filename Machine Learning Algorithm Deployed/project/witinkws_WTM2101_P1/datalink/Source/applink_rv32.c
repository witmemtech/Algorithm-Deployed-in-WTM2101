...
int applink_recv_heartbeat(applink_handle_t* self)
{
    ...
    self->link_ts_last_recv = self->ts;
    self->msg = NULL;
    return 1;
}
...
int applink_recv_file_info(applink_handle_t* self)
{
    ...
    //int nr_bytes;
    //datalink_msg_debug_t* dbg = (datalink_msg_debug_t*)&self->tx_dbg_msg;
    //nr_bytes = uartxfer_callback_debug(dbg->raw, DATALINK_MAX_PAYLOAD_LEN);
    //if (nr_bytes > DATALINK_MAX_PAYLOAD_LEN) {
    //    nr_bytes = DATALINK_MAX_PAYLOAD_LEN;
    //}

    //dbg->header.u.msgid = DATALINK_MSG_ID_DEBUG;
    //dbg->header.u.dlen  = nr_bytes;
    //datalink_encode(&self->tx_dbg_msg, self->link_tx_seq);
    //self->link_tx_seq++;
    //uart_send_msg(&self->tx_dbg_msg);
...
int applink_recv_file_data(applink_handle_t* self)
{
    ...
    uartxfer_callback_process(buff_in, buff_out, nr_bytes);
