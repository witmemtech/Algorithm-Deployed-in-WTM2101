...
static int aw_pid_2049_dsp_fw_check(struct aw_device *aw_dev)
{
	...
	dsp_fw_desc = aw883xx_dev_get_prof_data_byname(aw_dev, aw_dev->set_prof_name, AW_DATA_TYPE_DSP_FW);

	if (dsp_fw_desc == NULL) {
		aw_dev_err(aw_dev->dev, "dsp_fw_desc is null");
		return -EINVAL;
	}
...
int aw883xx_pid_2049_dev_init(void *aw883xx_val)
{
	int ret = -1;
	...
	if (aw883xx->chip_id != PID_2049_ID) {
		aw_dev_dbg(aw883xx->dev, "The id:0x%x does not match:0x%x",
					aw883xx->chip_id, PID_2049_ID);
		return -EINVAL;
	}

	aw_pa = pvPortMalloc(sizeof(struct aw_device));
	if (aw_pa == NULL) {
