/**
  ******************************************************************************
  * @file    main.c 
  * @brief   This example describes how to use the spi to transfer 
...
static void spi_test(void)
{
    ...
    SPI_Send_Receive(SPIM,1, master_send, sizeof(master_send),NULL,0);
    ...
    SPI_Send_Receive(SPIM,1, NULL, 0, master_receive, sizeof(master_receive));
    /* check data */
    if (memcmp(master_send, master_receive, sizeof(master_receive)) == 0)
    {
        printf("pass\r\n");
    } 
    else 
